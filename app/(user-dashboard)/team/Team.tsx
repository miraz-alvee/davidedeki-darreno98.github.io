...
interface CallRecord {
  ...
  reason: string;
  duration: string;
  status: "Active" | "Invited" | "Escalated";
}
...
const StatusBadge: React.FC<{ status: keyof typeof statusConfig }> = ({
  ...
  return (
    ...
    >
      <span className={`w-2 h-2 rounded-full ${cfg.dot} hidden md:block`} />
      {status}
    </span>
  );
};
...
const columns: ColumnsType<CallRecord> = [
  {
    title: <span className="text-gray-500 text-sm font-semibold">Member</span>,
    dataIndex: "name",
    key: "name",
    align: "center",
    render: (_, record) => (
  ...
  {
    title: <span className="text-gray-500 text-sm font-semibold">Role</span>,
    dataIndex: "role",
    key: "role",
    align: "center",
  ...
  {
    title: <span className="text-gray-500 text-sm font-semibold">Action</span>,
    dataIndex: "action",
    key: "action",
    align: "center",
    render: (_, record) => (
